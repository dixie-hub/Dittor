...
#include "feature/dirparse/dittor_ipc.h"
...
const token_rule_t routerdesc_token_table[] = {
  ...
  T01("dittor-proof", K_OPT_DITTOR_PROOF, GE(1), NO_OBJ),
                               ...
                               int *can_dl_again_out)
{
...

  {
    ...
    tor_cert_t *cert;
    {
      ...
      cert = tor_cert_parse((const uint8_t *)ed_cert_tok->object_body,
...

  {
    ...
    if (family_cert_toks) {
      r = check_family_certs(family_cert_toks,
                             ...
                             &router->family_ids, &family_expiration);
  ...
  // DITTOR SYBIL RESILIENT TOKEN VALIDATION
  directory_token_t *dittor_tok =
      find_opt_by_keyword(tokens, K_OPT_DITTOR_PROOF);
  if (dittor_tok && dittor_tok->n_args >= 1) {
    char ipc_req[4096];
    snprintf(ipc_req, sizeof(ipc_req), "VALIDATE %s\n", dittor_tok->args[0]);

    log_info(LD_DIR,
             "Dittor: Relaying token validation payload to Java Sidecar...");

    char *ipc_res = dittor_ipc_call(8081, ipc_req);

    if (!ipc_res || strcmp(ipc_res, "VALID") != 0) {
      log_warn(LD_DIR, "Dittor Proof Validation FAILED! Dropping untrusted "
                       "descriptor entry.");
      tor_free(ipc_res);
      goto err; 
    }

    log_notice(LD_DIR, "Dittor Proof verified successfully via local engine.");
    tor_free(ipc_res);
  } //end
 ...
 */
STATIC int
check_one_family_cert(const uint8_t *cert_body, size_t cert_body_size,
                      const ed25519_public_key_t *identity_key,
                      char **family_id_out, time_t *family_expiration_out)
{
...
check_family_certs(const smartlist_t *family_cert_tokens,
                   const ed25519_public_key_t *identity_key,
                   smartlist_t **family_ids_out, time_t *family_expiration_out)
{
  if (BUG(!identity_key) || BUG(!family_ids_out) ||
  ...
  *family_ids_out = smartlist_new();
  SMARTLIST_FOREACH_BEGIN (family_cert_tokens, directory_token_t *, tok) {
    ...
    if (check_one_family_cert((const uint8_t *)tok->object_body,
                              tok->object_size, identity_key, &this_id,
                              &this_expiration) < 0)
      goto err;
